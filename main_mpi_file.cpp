...
std::string serializer(std::map<std::string, int> counter, int rank){
    std::string filename = "serialized" + std::to_string(rank) + ".txt";
    freopen(filename.c_str(), "w", stdout);     
    for(auto x : counter){
        std::cout<<x.first + ':' + std::to_string(x.second) + '\n';
    }
...
int main(int argc, char **argv){
    ...
    int buffer_sz;
    std::string buf = serializer(words, rank);
    ...
    if(!rank){
        freopen("output_log_parallel_file.txt", "w", stdout);    
        if(order == 'a'){
